...
const newList = initialCountriesList.filter(each => each.isVisited === true)
...
class Countries extends Component {
  ...
  toggleVisited = id => {
    this.setState(
      ...
      this.makeVisitedCountriesList,
    )
  }

  makeVisitedCountriesList = () => {
    const {countriesList} = this.state
    const newListCountries = countriesList.filter(
      each => each.isVisited === true,
    )
    this.setState({visitedList: newListCountries})
  }
      ...
      <div className="main-container">
        ...
        {visitedList.length === 0 ? (
          ...
          <div className="countries-container">
            ...
            <ul className="countries-list-images">
              {visitedList.map(each => (
                <CountryImages
                  toggleIsVisited={this.toggleVisited}
                  visitedList={each}
                  key={each.id}
                />
              ))}
